...
generate_data <- function(n, p, m, true_beta, censoring_rate,
  ...
  # change last column of Z to intercept (column of ones)
  ...
  if(censoring_mechanism == "exog_broken"){
    ...
    # change this setting so that the errors will be equivalent regardless of censoring rate
  ...
  if(censoring_mechanism == "cond_indep_xz_broken"){
    ...
    e <- rnorm(n, 0, sqrt(sigma2))
    diffs = abs(e)
    logC = log(X) + sample(c(rep(-1, n*censoring_rate),
                             rep(1, n*(1-censoring_rate))), size = n)*diffs
    C = exp(logC)
    D <- ifelse(X <= C, 1, 0)
  ...
  D_mcar = (1:n %in% sample(n, n*(1-censoring_rate))) %>% as.numeric()

  # return data in data frame, including indicator for X censored, D
  data_yXZ <- cbind(y, X, Z, D, D_mcar) %>% as.data.frame()
  colnames(data_yXZ) <- if(is.null(Z)){c("y", "X", "D", "D_mcar")} else{c("y", "X", paste0("Z", 1:p), "D", "D_mcar")}
  return(data_yXZ)
}
